        ...
        <div>
            {countries.map((data, index) => (
                <div
                    ...
                    tabIndex={index}
                >
                    <div className="number inlineBlock" number={data.number}>
                        {data.number}
                    </div>
                    <div
...
function CountryNumber() {
    ...
    useEffect(() => {
        axios.get("/data/countryNumber.json").then((res) => {
    ...
    useEffect(() => {
        if (search === "") {
            ...
            setNation([
                ...countries
                    .filter((data) => data.country.indexOf(search) !== -1)
                    .sort(
                        (a, b) =>
                            a.country.indexOf(search) -
                            b.country.indexOf(search)
                    ),
            ]);
        }
    }, [search]);
