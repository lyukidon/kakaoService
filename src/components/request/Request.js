...
const reqData = yup.object({
    ...
    reqCategory2: yup
        ...
        .notOneOf(["선택해주세요"], "카테고리를 선택해주세요")
        .required("카테고리를 선택해주세요"),
    reqTitle: yup.string().required("제목을 입력해주세요."),
    reqContent: yup.string().required("내용을 입력해주세요."),
    reqAgree: yup.boolean().oneOf([true], "개인정보수집·이용에 동의해 주세요"),
});
...
const category = [
    ...
    {
        first: "일반문의",
        second: ["선택해주세요", "안드로이드", "iOS", "안드로이드(원스토어)"],
    },
    {
        first: "인증번호",
        second: [
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="formBox">
                ...
                <div className="dataInputBox categoryInput">
                    ...
                    <div className=" categoryBox inlineBlock">
                        ...
                        >
                            {select1 &&
                                category
                                    .filter((data) => data.first === select1)[0]
                                    .second.map((data) => (
                                        <option
                                            key={data}
                                            value={data}
            ...
            <div className="dataBox">
                ...
                <div className="dataInputBox oneInput">
                    ...
                    />
                    <div className="error">{errors.reqTitle?.message}</div>
                </div>
            </div>
            <div className="dataBox">
                ...
                <div className="dataInputBox oneInput contentInput">
                    <input
                        ...
                        {...register("reqContent", { required: true })}
                        type="text"
                        name="reqContent"
                        id=""
                    />
            ...
            <div className="dataPolicy">
                ...
                <div>
                    <strong>(필수) 개인정보 수집·이용에 대한 안내</strong>
                </div>
                <div>
                    주)카카오는 이용자 문의를 처리하기 위해 다음과 같이
                ...
                <div>
                    위 동의를 거부할 권리가 있으며, 동의를 거부하실 경우 문의
                    처리 및 결과 회신이 제한됩니다.
                </div>
                <div>
                    더 자세한 내용에 대해서는 카카오 개인정보처리방침을
                    참고하시기 바랍니다.
                </div>
                <div>
                    <input
                        ...
                        id="reqAgree"
                    />
                    <label htmlFor="reqAgree">위 내용에 동의합니다.</label>
                    <span className="error">{errors.reqAgree?.message}</span>
                </div>
            </div>
