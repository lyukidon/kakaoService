    ...
    return (
        <div id={`article${index}`} className="content" key={data.id}>
            ...
            <div className="contentLink">
                <Link
                    to={
                        query.articleId && +query.articleId === index
                            ? active
                            : inactiveUrl
                    }
                >
                ...
                {+query.articleId === index && <div>{data.explain}</div>}
            </div>
        </div>
    );
}
...
function Content({ query }) {
    ...
    useEffect(() => {
        axios.get("/data/faqMain.json").then((res) => {
            if (query.platform) {
                setArticle(
                    res.data.article[query.service][query.category][
                        query.platform
                    ]
                );
            } else {
                setArticle(res.data.article[query.service][query.category][0]);
            }
        });
        ...
        <div className="contentBox">
            {article.map((data, index) => (
                <div
                    key={data.id}
                    ref={(element) => {
                        positionRef.current[index] = element;
                    }}
                >
                    <Explain query={query} data={data} index={index} />
                </div>
            ))}
        </div>
    );
...
function Detail() {
    ...
    return (
        <div className="detail">
            <div className="categoryName">{category[query.category]}</div>
            <div className="platformBox">
                {platform[query.category] &&
                    platform[query.category].map((data, index, arr) => (
                        <div key={data}>
                            <Link
                                key={data}
                                className={
                                    +query.platform === index && "active"
                                }
